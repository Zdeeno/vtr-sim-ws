...
from torchrl.modules import ProbabilisticActor, TanhNormal, ValueOperator, LSTMModule, MLP
...
mlp = TensorDictModule(mlp, in_keys=["embed"], out_keys=["projection"])

prob_extract = TensorDictModule(NormalParamExtractor(), in_keys=["projection"], out_keys=["loc", "scale"])

policy_module = TensorDictSequential(lstm.set_recurrent_mode(True), mlp, prob_extract)
...
for i, tensordict_data in enumerate(collector):
    ...
    # scheduler.step()
    torch.save(policy_module.state_dict(), SAVE_DIR + "actor_net.pt")
